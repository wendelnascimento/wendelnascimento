...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ScrollOnClick = __webpack_require__(1);

	var _ScrollOnClick2 = _interopRequireDefault(_ScrollOnClick);
	...
	var _canvasLines = __webpack_require__(18);

	var _canvasLines2 = _interopRequireDefault(_canvasLines);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var introCanvas = new _canvasLines2.default(window.innerWidth, window.innerHeight, '#intro', '#canvas-lines', '2d', null, true);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var CanvasLines = function () {
		function CanvasLines(width, height, container, canvas, context, target, animateContainer) {
			_classCallCheck(this, CanvasLines);
		...
		_createClass(CanvasLines, [{
			...
			value: function initContainer() {
				...
				for (var x = 0; x < this._width; x = x + this._width / 20) {
					for (var y = 0; y < this._height; y = y + this._height / 20) {
						var px = x + Math.random() * this._width / 20;
						var py = y + Math.random() * this._height / 20;
						var p = {
							x: px,
							originX: px,
							y: py,
				...
				for (var i = 0; i < this._points.length; i++) {
					...
					for (var j = 0; j < this._points.length; j++) {
						...
						if (!(p1 == p2)) {
							...
							var c = new Circle(this._points[i], 2 + Math.random() * 5, 'rgba(0,0,0,0.3)');
							for (var _k = 0; _k < 5; _k++) {
								if (!placed) {
									if (c.getDistance(p1, p2) < c.getDistance(p1, closest[_k])) {
										closest[_k] = p2;
										placed = true;
									}
								}
				...
				for (var _i = 0; _i < this._points.length; _i++) {
					var _c = new Circle(this._points[_i], 2 + Math.random() * 5, 'rgba(0,0,0,0.3)');
					this._points[_i].circle = _c;
				}
			}
		}, {
			key: 'addListeners',
			value: function addListeners() {
				if (!('ontouchstart' in window)) window.addEventListener('mousemove', this.mouseMove.bind(this));
			...
			value: function scrollCheck() {
				if (document.body.scrollTop > this._height) this._animateContainer = false;else this._animateContainer = true;
			}
		}, {
			key: 'resize',
			...
			key: 'shiftPoint',
			value: function shiftPoint(p) {
				var _this = this;

				_gsap2.default.to(p, 3 + 2 * Math.random(), {
